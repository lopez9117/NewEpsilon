...
if (isset($_POST["phpmailer"])) {
    ...
    if ($mail->Send()) {
        if ($tipo_solicitud == 2) {
			...
			window.location="../listado/SolicitudesCompras.php";
				</script>';
        } else {
            echo '<script type="text/javascript">
				alert("Su solicitud fue enviada exitosamente");
			window.location="../listado/SolicitudesServicios.php";
				</script>';
        }
    } else {
        if ($tipo_solicitud == 2) {
...
mysql_query("INSERT INTO solicitud (idsolicitud, idsede, idarea, desc_requerimiento, fechahora_solicitud, fechahora_visita, idestado_solicitud, asunto, idfuncionario, idsatisfaccion, id_adquisicion, horasolicitud, horavisita, idfuncionarioresponde, idprioridad, id_tiposolicitud, id_estadocompra, id_presupuesto, idservicio, porque, id_referencia)
					VALUES ('$idSolicitud','$Sede','3','$requerimiento','$Fecha',NULL,'1','$asunt','$usuario','3','$tipo_Adquisicion','$time',NULL,'0','$prioridad','$tipo_solicitud','3','$tipo_Adquisicion','$servicio','','0')", $cn);
?>