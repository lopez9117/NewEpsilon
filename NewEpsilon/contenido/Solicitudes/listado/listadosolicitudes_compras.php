...
$cn = Conectarse();
$fecha_inicio=$_GET['fechaDesde'];
$fecha_limite=$_GET['fechaHasta'];
...
s.descsede, e.descestado_solicitud, CONCAT(f.nombres,'<br>', f.apellidos) AS nombre, p.desc_prioridad, so.id_tiposolicitud, so.id_presupuesto FROM solicitud so
...
INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='3' AND so.id_tiposolicitud='2' AND so.id_presupuesto!=0 OR so.id_tiposolicitud='0' AND so.idarea='3' AND so.id_presupuesto!=0 ORDER BY so.fechahora_solicitud DESC",$cn);
}
else if ($area==12)
{
	$listado =  mysql_query("SELECT so.idsolicitud, so.desc_requerimiento, so.fechahora_solicitud, so.horasolicitud, so.idsatisfaccion, so.idestado_solicitud,
...
INNER JOIN estado_solicitud e ON e.idestado_solicitud = so.idestado_solicitud
INNER JOIN funcionario f ON f.idfuncionario= so.idfuncionario
INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='3' AND so.id_tiposolicitud='2' AND so.idestado_solicitud='8' OR so.id_tiposolicitud='0' AND so.idestado_solicitud='8' AND so.idarea='3' ORDER BY so.fechahora_solicitud DESC",$cn);
}
else if ($area==5)
{
	$listado =  mysql_query("SELECT so.idsolicitud, so.desc_requerimiento, so.fechahora_solicitud, so.horasolicitud, so.idsatisfaccion, so.idestado_solicitud,
...
INNER JOIN estado_solicitud e ON e.idestado_solicitud = so.idestado_solicitud
INNER JOIN funcionario f ON f.idfuncionario= so.idfuncionario
INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='3' AND so.id_tiposolicitud='2' AND so.idestado_solicitud='5' OR so.id_tiposolicitud='0' AND so.idestado_solicitud='5' AND so.idarea='3' ORDER BY so.fechahora_solicitud DESC",$cn);
}
...
s.descsede, e.descestado_solicitud, CONCAT(f.nombres,'<br>', f.apellidos) AS nombre, p.desc_prioridad, so.id_tiposolicitud, so.id_estadocompra, so.id_presupuesto FROM solicitud so
INNER JOIN sede s ON s.idsede = so.idsede
INNER JOIN estado_solicitud e ON e.idestado_solicitud = so.idestado_solicitud
INNER JOIN funcionario f ON f.idfuncionario= so.idfuncionario
INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad
 WHERE so.idarea='3' AND so.id_tiposolicitud='0' OR so.idarea='3' AND so.id_tiposolicitud='2'",$cn);
}
else
{
	$listado =  mysql_query("SELECT so.idsolicitud, so.desc_requerimiento, so.fechahora_solicitud, so.horasolicitud, so.idsatisfaccion, so.idestado_solicitud,
...
INNER JOIN funcionario f ON f.idfuncionario= so.idfuncionario
INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad
 WHERE so.idarea='3' AND so.idfuncionario='$CurrentUser' AND so.id_tiposolicitud='2' OR so.id_tiposolicitud='0' AND so.idarea='3' AND so.idfuncionario='$CurrentUser' ORDER BY so.fechahora_solicitud DESC",$cn);
	}
?>
<script type="text/javascript">
 $(document).ready(function(){
   $('#tabla_listado_compras').dataTable( { //CONVERTIMOS NUESTRO LISTADO DE LA FORMA DEL JQUERY.DATATABLES- PASAMOS EL ID DE LA TABLA
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="tabla_listado_compras">
<thead>
    <tr>
        ...
        <th align="left" width="10%">Tipo Prioridad</th>
...
if ($area==5 and $reg[idestado_solicitud]==5 )
{
	   ...
	   if ($reg['id_estadocompra']==3 || $reg['id_estadocompra']==0)
			 {
	   ...
	   echo '<td align="left" bgcolor="FF8282">'.mb_convert_encoding($reg['desc_prioridad'], "UTF-8").'</td>';
	  ...
	  <input type="radio" name="'.$reg['idsolicitud'].'" id="opt" value="4" alt="En Tramite" title="En Tramite" onclick="enproceso('.$reg['idsolicitud'].')"><img src="../../../images/edit_remove.png" width="20" height="20" id="Image1" border="0" alt="En Tramite" title="En Tramite"><br>
		<input type="radio" name="'.$reg['idsolicitud'].'" id="opt" value="5" alt="Cumplido" title="Cumplido" onclick="cumplido('.$reg['idsolicitud'].')"><img src="../../../images/apply.png" width="20" height="20" id="Image1" border="0" alt="Cumplido" title="Cumplido">';
...
//validamos el estado que califica suministros para validar y dar por completado la solicitud
	   ...
	   echo '<td align="left" bgcolor="FFFF8A">'.mb_convert_encoding($reg['desc_prioridad'], "UTF-8").'</td>';
...
//validamos el estado que califica suministros para validar y dar por completado la solicitud
	   ...
	   echo '<td align="left" bgcolor="C8CBFD">'.mb_convert_encoding($reg['desc_prioridad'], "UTF-8").'</td>';
...
echo '<tr>';
		if ($reg['idestado_solicitud']==1 || $reg['idestado_solicitud']==8)
	   {
		...
		echo '<td align="left" bgcolor="E6EFF1">'.mb_convert_encoding($reg['desc_prioridad'], "UTF-8").'</td>';
		...
		while($regAdjunto = mysql_fetch_array($sqlAdjunto))
   {
	...
	echo '<a href="../insert/descarga.php?id='.$regAdjunto['adjunto'].'"><img src="../../../images/ark2.png" width="20" height="20" id="Image1" border="0" alt="Descargar Adjunto" title="Descargar Adjunto"></a>';
		...
		echo '<a href="../listado/detallecompras.php?id='.$reg['idsolicitud'].'&area=3" target="popup" onClick="window.open(this.href, this.target, width=300,height=400); return false;"><img src="../../../images/kfind.png" width="20" height="20" id="Image1" border="0" alt="Ver Detalles" title="Ver Detalles"></a>';
	   	echo '<a href="facturacompras.php?id='.$reg['idsolicitud'].'" target="popup" onClick="window.open(this.href, this.target, width=300,height=400); return false;"><img src="../../../images/fileprint.png" width="20" height="20" id="Image1" border="0" alt="Imprimir" title="Imprimir"></a>';
	if ($reg['id_estadocompra']==4)
		{
		echo '<img src="../../../images/edit_remove.png" width="20" height="20" id="Image1" border="0" alt="En Tramite" title="En Tramite">';
}
else if ($reg['id_estadocompra']==5)
{
	echo '<img src="../../../images/apply.png" width="20" height="20" id="Image1" border="0" alt="Cumplido" title="Cumplido">';
}
		'</td>';
		}
