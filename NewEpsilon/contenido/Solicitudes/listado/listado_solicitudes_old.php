...
INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='1' AND fechahora_solicitud BETWEEN '$DESDE' AND '$HASTA' ORDER BY so.fechahora_solicitud DESC",$cn);
}
...
INNER JOIN estado_solicitud e ON e.idestado_solicitud = so.idestado_solicitud
INNER JOIN funcionario f ON f.idfuncionario= so.idfuncionario
INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='1' AND so.idfuncionario='$CurrentUser' AND idsatisfaccion='$Satisfaccion' AND so.idestado_solicitud>=3 ORDER BY so.fechahora_solicitud DESC",$cn);
}
...
INNER JOIN estado_solicitud e ON e.idestado_solicitud = so.idestado_solicitud
INNER JOIN funcionario f ON f.idfuncionario= so.idfuncionario
INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='1' AND so.idfuncionario='$CurrentUser' AND idsatisfaccion='$Satisfaccion' ORDER BY so.fechahora_solicitud DESC",$cn);
}
...
INNER JOIN estado_solicitud e ON e.idestado_solicitud = so.idestado_solicitud
INNER JOIN funcionario f ON f.idfuncionario= so.idfuncionario
INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='1' AND so.idfuncionario='$CurrentUser' ORDER BY so.fechahora_solicitud DESC",$cn);
}
...
{
?>
 <a  href="../solicitudSistemas.php?usuario=<?php echo base64_encode($CurrentUser)?>" target="pop-up" onClick="window.open(this.href, this.target, 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, width=1900, height=800, top=0, left=140'); return false;" class="btn btn-primary btn-sm">Realizar Solicitud</a>
<?php
}
?>
</div>