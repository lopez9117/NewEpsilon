...
INNER JOIN funcionario f ON s.idfuncionario = f.idfuncionario WHERE idarea=1 and idsatisfaccion=3 and s.idfuncionario='$CurrentUser' AND idestado_solicitud!=1 AND idestado_solicitud != '2' ", $cn);
$regusuario = mysql_fetch_array($consulta);
$count = mysql_num_rows($consulta);
if ($area == 1 || $area == 3 || $area == 6) {
    ...
    s.descsede, e.descestado_solicitud,CONCAT(f.nombres,' ', f.apellidos) AS nombre, so.idfuncionario, p.desc_prioridad FROM solicitud so
    ...
    INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='1' AND fechahora_solicitud BETWEEN '$DESDE' AND '$HASTA' ORDER BY so.fechahora_solicitud ASC", $cn);
    ...
    INNER JOIN estado_solicitud e ON e.idestado_solicitud = so.idestado_solicitud
    INNER JOIN funcionario f ON f.idfuncionario= so.idfuncionario
    INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='1' AND so.idfuncionario='$CurrentUser' AND idsatisfaccion='$Satisfaccion' AND so.idestado_solicitud>=3 ORDER BY so.fechahora_solicitud ASC", $cn);
    ...
    INNER JOIN estado_solicitud e ON e.idestado_solicitud = so.idestado_solicitud
    INNER JOIN funcionario f ON f.idfuncionario= so.idfuncionario
    INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='1' AND so.idfuncionario='$CurrentUser' AND idsatisfaccion='$Satisfaccion' ORDER BY so.fechahora_solicitud ASC", $cn);
    ...
    INNER JOIN estado_solicitud e ON e.idestado_solicitud = so.idestado_solicitud
    INNER JOIN funcionario f ON f.idfuncionario= so.idfuncionario
    INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='1' AND so.idfuncionario='$CurrentUser' ORDER BY so.fechahora_solicitud ASC", $cn);
}

?>
<script type="text/javascript">
    $(document).ready(function () {
        $('#tabla_listado_funcionarios').dataTable({
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="tabla_listado_funcionarios">
    ...
    <tr align="center">
        ...
        <th width="10%">Fecha / Hora Solicitud</th>
        <th width="5%">Prioridad:</th>
        <th width="5%%">Estado</th>
        <th width="5%%">Satisfacción</th>
        <th width="10%">Solucionado por</th>
    ...
    while($reg = mysql_fetch_array($listado))
    {
        $CurrentUser = $_SESSION['currentuser'];
        $id = $reg['idsolicitud'];
        $list= mysql_query("SELECT so.idsolicitud, CONCAT(d.nombres,' ', d.apellidos) AS nombre FROM solicitud so
        ...
        echo '<td>'.ucwords(strtolower($reg['nombre'])).'</td>';
        echo '<td>'.$FECHASOL.' / '.$reg['horasolicitud'].'</td>';
        echo '<td>'.ucwords(strtolower($reg['desc_prioridad'])).'</td>';
        echo '<td>'.ucwords(strtolower($reg['descestado_solicitud'])).'</td>';
        ...
        {
            echo '<td align="center">Si<input type="radio" name="'.$reg['idsolicitud'].'" id="opt" value="1" onClick="satisfecho('.$reg['idsolicitud'].')" checked="checked">No<input type="radio" name="'.$reg['idsolicitud'].'" id="opt" value="2" onClick="nosatisfecho('.$reg['idsolicitud'].')"></td>';
        }
        if ($reg['idsatisfaccion']==2) {
        echo '<td align="center">Si<input type="radio" name="'.$reg['idsolicitud'].'" id="opt" value="1" onClick="satisfecho('.$reg['idsolicitud'].') ">No<input type="radio" name="'.$reg['idsolicitud'].'" id="opt" value="2" onClick="nosatisfecho('.$reg['idsolicitud'].')" checked="checked"></td>';
        }
        }
        else{
        if ($reg['idsatisfaccion']==1)
        ...
        {
        echo '<td align="left">'.ucwords(strtolower($reg2['nombre'])).'</td>';
        echo '<td></td>';
        }
        echo '<td align="center">';
        ...
        <a href="detalle.php?id=<?php echo $reg['idsolicitud']?>&area=1" target="pop-up" onClick="window.open(this.href, this.target, 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, width=1920, height=1080, top=85, left=140'); return false;"><img src="../../../images/kfind.png" width="20" height="20" id="Image1" border="0" alt="Imprimir" title="Imprimir"></a>
        </td>
        </tr>
    <?php
    }
...
<div class="row" aling="left">
    ...
    if ($count >= 1) {
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <h4>Por favor califique todas las solicitudes resueltas por el departamente de sistemas.</h4>
        ...
        ?>
        <a href="../solicitudSistemas.php?usuario=<?php echo base64_encode($CurrentUser) ?>" target="pop-up"
           onClick="window.open(this.href, this.target, 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, width=1900, height=800, top=0, left=140'); return false;"
           class="btn btn-primary btn-sm">Realizar Solicitud</a>
        <?php
    }
    ?>
</div>