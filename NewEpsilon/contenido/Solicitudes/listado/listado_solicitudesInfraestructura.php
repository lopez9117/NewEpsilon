...
INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='8' AND fechahora_solicitud BETWEEN '$DESDE' AND '$HASTA' ORDER BY so.fechahora_solicitud DESC",$cn);
}
...
INNER JOIN estado_solicitud e ON e.idestado_solicitud = so.idestado_solicitud
INNER JOIN funcionario f ON f.idfuncionario= so.idfuncionario
INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='8' AND so.idfuncionario='$CurrentUser' AND idsatisfaccion='$Satisfaccion' AND so.idestado_solicitud>=3 ORDER BY so.fechahora_solicitud DESC",$cn);
}
...
INNER JOIN estado_solicitud e ON e.idestado_solicitud = so.idestado_solicitud
INNER JOIN funcionario f ON f.idfuncionario= so.idfuncionario
INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='8' AND so.idfuncionario='$CurrentUser' AND idsatisfaccion='$Satisfaccion' ORDER BY so.fechahora_solicitud DESC",$cn);
}
...
INNER JOIN estado_solicitud e ON e.idestado_solicitud = so.idestado_solicitud
INNER JOIN funcionario f ON f.idfuncionario= so.idfuncionario
INNER JOIN tipo_prioridad p ON p.idprioridad= so.idprioridad WHERE so.idarea='8' AND so.idfuncionario='$CurrentUser' ORDER BY so.fechahora_solicitud DESC",$cn);
}
...
INNER JOIN solicitud so ON so.idsolicitud = ad.idsolicitud where so.idsolicitud='$id'", $cn);
	...
	while($regAdjunto = mysql_fetch_array($sqlAdjunto))
   {
	...
	echo '<a href="../insert/descarga.php?id='.$regAdjunto['adjunto'].'"><img src="../../../images/ark2.png" width="20" height="20" id="Image1" border="0" alt="Descargar Adjunto" title="Descargar Adjunto"></a>';
      ...
      <a href="detalle.php?id=<?php echo $reg['idsolicitud']?>&area=8" target="pop-up" onClick="window.open(this.href, this.target, 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, width=1920, height=1080, top=85, left=140'); return false;"><img src="../../../images/kfind.png" width="20" height="20" id="Image1" border="0" alt="Imprimir" title="Imprimir"></a>
   </td>
       </tr>
       <?php
	   }
...
{
?>
 <a  href="../SolicitudInfraestructura.php?usuario=<?php echo base64_encode($CurrentUser)?>" target="pop-up" onClick="window.open(this.href, this.target, 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, width=1900, height=800, top=0, left=140'); return false;" class="btn btn-primary btn-sm">Realizar Solicitud</a>
<?php
}
?>
</div>