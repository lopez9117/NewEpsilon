...
function GetEstadoVentana($cn, $idInforme,$idfuncionario){
    $sqlEstado = mysql_query("SELECT * FROM r_estadoventana WHERE id_informe = '$idInforme' AND idfuncionario!='$idfuncionario'", $cn);
    $ContEstado = mysql_num_rows($sqlEstado);
    if($ContEstado >=1){
        $boolReturn = true;
    }
    else{
...
function GetRegistrarEstado($cn,$idinforme,$idfuncionario) {
    mysql_query("INSERT INTO r_estadoventana (id_informe, idfuncionario) VALUES ('$idinforme','$idfuncionario')",$cn);
}
function GetEliminarEstado($cn,$idinforme){
    $type='hola';
