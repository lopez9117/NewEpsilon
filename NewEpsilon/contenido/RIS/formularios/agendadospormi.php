...
for($h=7;$h<=19;$h++){
	...
	for($m = $m - 60;$m <= 59;$m+=7){
		$horas_estaticas = ($h < 9?'0':'').$h.':'.($m < 9 ?'0':'').':00'.$m;
	}
}
...
{
	if(in_array( $reg['hora'],$horas_estaticas))
	{
		//Si la hora de la base de datos esta asignada, se muestra aquí
	...
	{
		//De lo contrario, mostrar el registro vacío listo para asignarse
		   echo '<td align="left">' . $reg['hora'] . '</td>';
		...
		echo$horas_estaticas array($index);
		
		// colocar todo igual pero mostrar a qui solo horas_estatica
