...
include_once('config.inc.php');
	...
	//check if partner is offline, echo it on the end
	...
	if(mysql_num_rows($check_offline) > 0){
		$echo_offline = '<p><span class="error">El usuario esta desconectado!</span></p>';
	}
	...
	if(mysql_num_rows($res) > 0){
		...
		//echo last message time if older than 2 mins
		$math = time() - $last_msg;
		if($math > 120){
			echo '<p class="system">Ultimo mensaje enviado a las: '.date('H:i').'</p>';
		}
