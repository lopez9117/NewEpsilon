...
$cn = Conectarse();
//Declaraciones de indices de variables
base64_decode($_GET['FchDesde']); base64_decode($_GET['FchHasta']); base64_decode($_GET['sede']);
...
header("content-disposition: attachment;filename = ReporteRadiacion" . $FchDesde . '/' . $FchHasta . ".xls");
//recorrer el vector servicios
foreach($Servicios as $idServicio => $DescripcionServicio){
    ...
    if($ContEstudios>=1){
        ...
        <tr align="center" class="header">
            ...
            <td>MAS</td>
            <td>KV</td>
            <td>Imagenes Dañadas</td>
            <td>Radiacciones Innecesarias</td>
            <td>Realizado Por</td>
            <td>Fecha / Hora Realización</td>
...
function GetDetalles($IdInforme, $cn){
    ...
    $SqlDosisRadiacion = mysql_query("SELECT DISTINCT(e.id_informe), e.mas, e.kv, e.i_danadas, e.r_innecesarias FROM r_estadistica e
    ...
    $String .= '<td>'.$RegDetalle['id_paciente'].'</td>
            ...
            <td>'.$RegDosisRadiacion['i_danadas'].'</td>
