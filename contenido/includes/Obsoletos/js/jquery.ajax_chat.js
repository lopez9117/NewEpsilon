		...
		function liveChat(){
			...
			$('.chatbox').each(function(){												
				...
				$.ajax({
					...
					success: function(e){
						if(e == 1){
							$('.chatbox[title="'+this_chatbox_id+'"] .chat_info p').text('Escribiendo...');
						}else{
			...
			function PopupChat(partner_id,partner_username,chatbox_status){
				...
				$('body').append('<div class="chatbox cb_default" id="chat_window_'+wctr+'" title="'+partner_id+'">'+
					'<div class="header header_bg_default" title="'+partner_username+'">'+
						...
						'<a href="#" class="close_chatbox" title="Cerrar Ventana">X</a>'+
						'<a href="#" class="minimize_chatbox" title="Minimizar Ventana">__</a>'+
						'<a href="#" class="maximize_chatbox" title="Maximizar Ventana">&#8254;</a>'+
					'</div>'+
					'<div class="chat_area" title="'+partner_username+'">'+
					'</div>'+
					'<div class="chat_info"><p></p></div>'+					
					'<div class="chat_message" title="Escriba aqui su mensaje">'+
						'<textarea></textarea>'+
					'</div>'+
				'</div>');
				if (chatbox_status == 2) {
