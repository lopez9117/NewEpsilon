...
header("Content-Disposition: attachment; filename=ConsolidadoInfraestructura.xls");
...
left JOIN tipo_solicitud t ON t.id_tiposolicitud = so.id_tiposolicitud
left JOIN sede s ON s.idsede = so.idsede
left JOIN estado_solicitud e ON e.idestado_solicitud = so.idestado_solicitud
left JOIN funcionario f ON f.idfuncionario= so.idfuncionario
left JOIN satisfaccion sa ON sa.idsatisfaccion= so.idsatisfaccion
left JOIN tipo_prioridad tp ON tp.idprioridad=so.idprioridad
WHERE so.idarea = '8' AND fechahora_solicitud BETWEEN '$DESDE' AND '$HASTA' ORDER BY so.fechahora_solicitud DESC",$cn);
 ?>
