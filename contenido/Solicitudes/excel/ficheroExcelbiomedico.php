...
header("Content-Disposition: attachment; filename=ConsolidadoBiomedico.xls");
...
s.descsede, e.descestado_solicitud,CONCAT(f.nombres,' ', f.apellidos) AS nombres, sa.desc_satisfaccion,tp.desc_prioridad FROM solicitud so
...
WHERE so.idarea = '2' AND fechahora_solicitud BETWEEN '$DESDE' AND '$HASTA' ORDER BY so.fechahora_solicitud DESC",$cn);
 ?>
