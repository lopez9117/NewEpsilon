...
function mostrarAgenda() {
    ...
    if (fecha == "" || sede == "" || servicio == "" || estado == "") {
        ...
        function verlistado() { //FUNCION PARA MOSTRAR EL LISTADO EN EL INDEX POR JQUERY
            ...
            $.post("lista_pacientes_agendados.php?fecha=" + fecha + "&sede=" + sede + "&servicio=" + servicio + "&usuario=" + usuario + "&estado=" + estado + "", {
            ...
            <td width="28%">Sede:<br><label for="sede"></label>
                <select name="sede" id="sede" onChange="mostrarAgenda()">
                    ...
                    <?php while ($rowSede = mysql_fetch_array($listaSede)) { ?>
                        <option value="<?php echo $rowSede['idsede'] ?>" <?php if ($rowSede['idsede'] == $sede) { echo 'selected'; } ?>><?php echo $rowSede['descsede'] ?></option>
                    <?php } ?>
