...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="Lista_Pacientes">
    ...
    <?php foreach ($registros as $reg):?>
        ...
        {            
                ...
                target="pop-up"
                 onClick="window.open(this.href, this.target, 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, width=1520, height=1080, top=85, left=140'); return false;"
                 class="boton">Agendar Paciente</a></td>
                <?php
            }
            else if( $estado == 1 || $estado == 6)   {  if ($reg['id_estadoinforme'] > 1 || $reg['id_estadoinforme'] == 6) {
